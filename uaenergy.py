...
def parse_news(link:str, use_date:bool = True):
    ...
    df_articles = pd.DataFrame([parse_article(x) for x in articles], columns=["Title", "Link", "Date"])
...
def get_article_content(link:str, add_root:bool = True, warnings:bool = False):
    ...
    if article_object is None:
        if warnings: print("No article found:", link)
        return link, None, None, None
        
    read_also = [tag.a.get("href") for tag in article_object.find_all("p")
                 if "ЧИТАЙТЕ ТАКОЖ" in tag.text and tag.find('a') is not None]
    if len(read_also) == 0: read_also = None
    
    article_text = [tag.text for tag in article_object.find_all("p") if not "ЧИТАЙТЕ ТАКОЖ" in tag.text]
    ...
    try:
        ...
        if warnings: print("No tags found:", link)
        tags = None
